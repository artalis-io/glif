...
UTEST(font, create_fails_with_nonexistent_font) {
    ...
    GlifCharDatabase db;
    int ret = glif_char_db_create(&db, "fonts/nonexistent.ttf", 10, 20, &sc);
    ASSERT_NE(ret, 0);
}
...
UTEST(font, create_fails_with_invalid_font_data) {
    ...
    GlifCharDatabase db;
    int ret = glif_char_db_create(&db, "/tmp/glif_test_badfont.ttf", 10, 20, &sc);
    ASSERT_NE(ret, 0);
...
UTEST(font, space_has_zero_shape_vector) {
    ...
    /* Space is entry 0 (GLIF_CHAR_FIRST = 32, space = 32) */
    ASSERT_EQ(db.entries[0].ch, ' ');
    ASSERT_NEAR(glif_vec6_length(db.entries[0].shape), 0.0f, 1e-9f);
...
UTEST(font, nonspace_chars_have_nonzero_shapes) {
    ...
    int idx_A = 'A' - GLIF_CHAR_FIRST;
    ASSERT_EQ(db.entries[idx_A].ch, 'A');
    ASSERT_GT(glif_vec6_length(db.entries[idx_A].shape), 0.01f);

    /* 'M' = 77, index 45 */
    int idx_M = 'M' - GLIF_CHAR_FIRST;
    ASSERT_EQ(db.entries[idx_M].ch, 'M');
    ASSERT_GT(glif_vec6_length(db.entries[idx_M].shape), 0.01f);

    /* '#' = 35, index 3 */
    int idx_hash = '#' - GLIF_CHAR_FIRST;
    ASSERT_EQ(db.entries[idx_hash].ch, '#');
    ASSERT_GT(glif_vec6_length(db.entries[idx_hash].shape), 0.01f);
...
UTEST(font, all_95_entries_populated) {
    ...
    ASSERT_EQ(GLIF_CHAR_COUNT, 95);

    for (int i = 0; i < GLIF_CHAR_COUNT; i++) {
        ASSERT_EQ(db.entries[i].ch, (char)(GLIF_CHAR_FIRST + i));
...
UTEST(font, render_bitmaps_valid_scale) {
    ...
    uint8_t **bitmaps = glif_char_db_render_bitmaps(&db, 2);
...
UTEST(font, render_bitmaps_scale1_vs_scale2_differ) {
    ...
    uint8_t **bmp1 = glif_char_db_render_bitmaps(&db, 1);
    uint8_t **bmp2 = glif_char_db_render_bitmaps(&db, 2);
    ...
    for (int i = 0; i < GLIF_CHAR_COUNT; i++) free(bmp1[i]);
    free(bmp1);
    for (int i = 0; i < GLIF_CHAR_COUNT; i++) free(bmp2[i]);
    free(bmp2);
